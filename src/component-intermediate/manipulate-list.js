...
const ListItem = function ({onRemove, onUpdate, item: {id, value}}) {
  ...
  return (
    <div style={{border: "1px solid black", margin: "20px", padding: "20px"}}>
      ...
      <button onClick={() => onRemove(id) /* 필요한 인자값(삭제할 id) 전달 */}>
        remove
      </button>
      &nbsp;
      <button
...
const ListComponent = function (props) {
  ...
  const handleCreateItem = () =>
    setList(state => state.concat({id: v4(), value: 0}));

  // remove (filter)
  // id가 일치할 경우 false 리턴하여 리스트에서 제외
  const handleRemoveItem = uuid =>
    setList(state => state.filter(item => item.id !== uuid));
    
  // update (map)
  // id가 일치할 경우 수정된 객체를 반환
  const handleUpdateItem = (uuid, updated) =>
    setList(state => state.map(item => (item.id === uuid ? updated : item)));
